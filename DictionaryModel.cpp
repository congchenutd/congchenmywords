...
QString DictionaryModel::getUnitRangeFilter(const Filter& flt)
{
	...
	return tr("(id BETWEEN %1 AND %2)").arg(minID).arg(maxID);
}
...
QString DictionaryModel::getCreditRangeFilter(const Filter& flt)
{
	QStringList clauses;
	if(flt.showNew)
		clauses << tr("(credit<%1)").arg(flt.threshold1);
	if(flt.showUnFamiliar)
		clauses << tr("(credit BETWEEN %1 AND %2)").arg(flt.threshold1).arg(flt.threshold2-1);
	if(flt.showFamiliar)
		clauses << tr("(credit>=%1)").arg(flt.threshold2);
...
{
	QSqlQuery query;
	query.exec(tr("UPDATE %1 SET sorting=random() WHERE %2").arg(dictName)
...
int DictionaryModel::getWordCount(bool newWords, bool unfamiliar, bool familiar) const
{
	...
	QSqlQuery query;
	query.exec(tr("SELECT count(id) FROM %1 WHERE %2").arg(dictName)
...
{
	QSqlQuery query;
	query.exec(tr("UPDATE %1 SET credit=0").arg(dictName));
	query.exec(tr("UPDATE %1 SET credit=(SELECT %2.credit FROM %2 WHERE %2.word=%1.word) \
				   WHERE word in (SELECT %1.word FROM %1, %2 WHERE %2.word=%1.word)")
