...
void MainWindow::initModel()
{
	...
	progressModel->setData(progressModel->index(0, 0), tr("New"));
	progressModel->setData(progressModel->index(1, 0), tr("Learning"));
	progressModel->setData(progressModel->index(2, 0), tr("Learned"));
}
...
void MainWindow::initConnection()
{
	...
	connect(ui.actionLoadDict,   SIGNAL(triggered()), this, SLOT(slotLoadDict()));
	connect(ui.actionDelDict,    SIGNAL(triggered()), this, SLOT(slotDelDict()));
	connect(ui.actionNewDict,    SIGNAL(triggered()), this, SLOT(slotNewDict()));
	...
	connect(ui.actionAddWord,    SIGNAL(triggered()), this, SLOT(slotAddWord()));
...
void MainWindow::loadSizes()
{
	...
	ui.dockWidgetGroup->setMinimumWidth(width() * 0.25);
    ui.dockWidgetCognate->setMaximumHeight(height() * 0.55);
}
...
void MainWindow::slotDelDict()
{
	if(ui.comboBox->currentIndex() >= 0 && 
	   QMessageBox::warning(this, tr("Warning"), tr("Are you sure to delete?"),
						    QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes)
	{
		Library::delDict(getCurrentDictName());
		updateDictList();
...
void MainWindow::slotResizeUnit(int size)
{
	...
	const int unitCount = size > 0 ? ceil((float)wordCount/size) : 0;
...
void MainWindow::slotAbout() {
	QMessageBox::about(this, tr("About"), 
		tr("<h3><b>MyWords - How I recite words</b></h3>"
		   "<p>Cong Chen, 2010.6.26</p>"
		   "<p>Free for noncommercial use</p>"
		   "<a href=http://www.utdallas.edu/~cxc094020>Website</a>"
		   "<p><a href=mailto:congchenutd@gmail.com>Email</a></p>"));
}
...
void MainWindow::slotExport()
{
	...
	QProgressDialog progress(tr("Exporting %1 words").arg(count), 
							 0, 0, count);
...
{
	bool ok;
	QString dictName = QInputDialog::getText(this, tr("New dictionary"),
		tr("Dictionary name:"), QLineEdit::Normal, tr("NewDictionary"), &ok);
	if (ok && !dictName.isEmpty())
	{
		Library::createDictTable(dictName);
...
{
	WordEditDlg dlg(this);
	if(dlg.exec() == QDialog::Accepted)
	{
		Library::addWord(getCurrentDictName(), 
				dlg.getWord(), 
				dlg.getPhonetic(), 
				dlg.getExplanation(),
				dlg.getNote());
